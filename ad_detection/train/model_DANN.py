...
def print_dann_model_info(model, feature_type="XLSR"):
    """
    Print DANN model architecture information
    
    Args:
        model: DANN model instance
        feature_type: "XLSR" or "eGeMAPS"
    """
    print(f"\n{'='*60}")
    print(f"DANN Model Architecture ({feature_type})")
    print(f"{'='*60}")
    ...
    feature_params = sum(p.numel() for name, p in model.named_parameters() 
                        if 'class_classifier' not in name and 'domain_classifier' not in name)
    class_params = sum(p.numel() for name, p in model.named_parameters() 
                      if 'class_classifier' in name)
    domain_params = sum(p.numel() for name, p in model.named_parameters() 
                       if 'domain_classifier' in name)
    
    print(f"Total Parameters: {total_params:,}")
    print(f"Trainable Parameters: {trainable_params:,}")
    print(f"\nParameter Distribution:")
    print(f"  - Feature Extractor: {feature_params:,} ({feature_params/total_params*100:.1f}%)")
    print(f"  - Class Classifier: {class_params:,} ({class_params/total_params*100:.1f}%)")
    print(f"  - Domain Classifier: {domain_params:,} ({domain_params/total_params*100:.1f}%)")
    print(f"{'='*60}\n")


if __name__ == "__main__":
    # Test the models
    print("Testing DANN Models...")
    
    # Test XLSR model
    print("\n1. Testing AD_XLSR_Model_DANN")
    xlsr_model = AD_XLSR_Model_DANN(dropout=0.2)
    
    # Create dummy input
    batch_size = 4
    seq_len = 100
    xlsr_features = torch.randn(batch_size, seq_len, 1024)
    xlsr_mask = torch.ones(batch_size, seq_len)
    
    # Test forward pass
    print("   Forward pass (inference mode)...")
    class_logits = xlsr_model(xlsr_features, xlsr_mask, return_features=False)
    print(f"   Class logits shape: {class_logits.shape}")  # Should be (4, 2)
    
    print("   Forward pass (training mode)...")
    class_logits, domain_logits = xlsr_model(xlsr_features, xlsr_mask, alpha=0.5, return_features=True)
    print(f"   Class logits shape: {class_logits.shape}")  # Should be (4, 2)
    print(f"   Domain logits shape: {domain_logits.shape}")  # Should be (4, 2)
    
    print_dann_model_info(xlsr_model, "XLSR")
    
    # Test eGeMAPS model
    print("\n2. Testing AD_EGE_Model_DANN")
    ege_model = AD_EGE_Model_DANN(dim_input=25, dim_hidden=14, dropout=0.2)
    
    # Create dummy input
    ege_features = torch.randn(batch_size, 10, 25)
    
    # Test forward pass
    print("   Forward pass (inference mode)...")
    class_logits = ege_model(ege_features, return_features=False)
    print(f"   Class logits shape: {class_logits.shape}")  # Should be (4, 2)
    
    print("   Forward pass (training mode)...")
    class_logits, domain_logits = ege_model(ege_features, alpha=0.5, return_features=True)
    print(f"   Class logits shape: {class_logits.shape}")  # Should be (4, 2)
    print(f"   Domain logits shape: {domain_logits.shape}")  # Should be (4, 2)
    
    print_dann_model_info(ege_model, "eGeMAPS")
    
    # Test alpha computation
    print("\n3. Testing Alpha Schedule")
    total_steps = 1000
    test_steps = [0, 100, 250, 500, 750, 900, 999]
    print(f"   Total steps: {total_steps}")
    for step in test_steps:
        alpha = compute_alpha(step, total_steps)
        print(f"   Step {step:4d}: alpha = {alpha:.4f}")
