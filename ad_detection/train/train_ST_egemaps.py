"""
Self-Training Functions for eGeMAPS Domain Adaptation

This module implements the self-training algorithm for eGeMAPS features:
...
Key differences from XLSR ST:
- Uses AD_EGE_Model instead of AD_XLSR_Model
- No mask/padding needed (eGeMAPS features are fixed-size vectors)
- Simpler feature handling
"""
...
import numpy as np
from config import LEARNING_RATE, MAX_EPOCHS, WEIGHT_DECAY, EGEMAPS_DROPOUT
...
class PseudoLabelDataset_EGE(torch.utils.data.Dataset):
    """
    Dataset for eGeMAPS target domain samples with pseudo-labels
    
    Args:
        features_list: List of torch tensors (25,) for eGeMAPS
    ...
    def __init__(self, features_list, pseudo_labels_list, session_ids_list, confidences_list=None):
        ...
        if len(self.features) > 0:
            ...
            print(f"PseudoLabelDataset (eGeMAPS): {len(self)} samples")
            print(f"  Control (pseudo): {num_control}, Dementia (pseudo): {num_dementia}")
...
def create_st_dataloaders_egemaps(source_dataset, pseudo_dataset, batch_size=12, seed=None):
    """
    Create combined dataloader for eGeMAPS self-training
    
    Args:
        source_dataset: FeatureDataset with real source labels
        pseudo_dataset: PseudoLabelDataset_EGE with pseudo-labels (can be empty)
    ...
    if len(pseudo_dataset) == 0:
        ...
        print("Warning: No pseudo-labels, using source only")
        combined_dataset = source_dataset
    else:
        combined_dataset = torch.utils.data.ConcatDataset([source_dataset, pseudo_dataset])
    ...
    combined_loader = torch.utils.data.DataLoader(
        ...
        shuffle=True,
        num_workers=0,  # eGeMAPS is lightweight, no need for multi-workers
...
def generate_pseudo_labels_egemaps(model, target_dataset, device, confidence_threshold=ST_CONFIDENCE_THRESHOLD):
    """
    Generate pseudo-labels for eGeMAPS target domain data
    ...
    with torch.no_grad():
        for idx in tqdm(range(len(target_dataset)), desc="Pseudo-labeling", leave=False):
            ...
            features = features.unsqueeze(0).to(device)  # (1, 25)
            
            # Forward pass (eGeMAPS doesn't need mask)
    ...
    print(f"  Control: {class_0_count}, Dementia: {class_1_count}")
    
    return selected_features, selected_labels, selected_session_ids, selected_confidences, stats


def train_st_iteration_egemaps(iteration, model, source_dataset, pseudo_dataset, source_val_loader, target_val_loader,
                                device, seed, max_epochs=MAX_EPOCHS, patience=ST_PATIENCE,
                                batch_size=12, source_weight=0.5, target_weight=0.5):
    """
    Train eGeMAPS model for one ST iteration on source + pseudo-labeled data
    
    Args:
        iteration: Current ST iteration number (0 = initial training on source only)
        model: AD_EGE_Model (freshly initialized or loaded)
        source_dataset: FeatureDataset with real source labels
        pseudo_dataset: PseudoLabelDataset_EGE with pseudo-labels (can be empty for iteration 0)
    ...
    train_loader = create_st_dataloaders_egemaps(source_dataset, pseudo_dataset, batch_size=batch_size, seed=seed)
    ...
    print(f"\n{'='*60}")
    print(f"ST Iteration {iteration} - Training (eGeMAPS)")
    print(f"Model Selection: Avg Acc (Source {source_weight:.1f}, Target {target_weight:.1f})")
    print(f"{'='*60}")
    
    for epoch in range(1, max_epochs + 1):
        # Train
        train_loss, train_acc = train_one_epoch(model, train_loader, optimizer, device, epoch=epoch)
...
def train_st_egemaps(seed, source_train_dataset, target_train_dataset, source_val_loader, target_val_loader,
                     ...
                     batch_size=12, dim_input=25, dim_hidden=32, dropout=EGEMAPS_DROPOUT,
                     source_weight=0.5, target_weight=0.5):
    """
    Complete Self-Training pipeline for eGeMAPS
    ...
    Args:
        ...
        batch_size: Batch size
        dim_input: eGeMAPS input dimension (default: 25)
    ...
    print(f"\n{'='*70}")
    print(f"Self-Training (eGeMAPS): Seed {seed}")
    print(f"Model Selection: Avg Acc (Source {source_weight:.1f}, Target {target_weight:.1f})")
    ...
    model = AD_EGE_Model(dim_input=dim_input, dim_hidden=dim_hidden, dropout=dropout).to(device)
    ...
    for iteration in range(max_iterations + 1):  # 0 to max_iterations inclusive
        ...
        if iteration == 0:
            ...
            selected_features, selected_labels, selected_ids, selected_confs, pseudo_stats = generate_pseudo_labels_egemaps(
            ...
            pseudo_dataset = PseudoLabelDataset_EGE(selected_features, selected_labels, selected_ids, selected_confs)
        ...
        trained_model, metrics, history = train_st_iteration_egemaps(
            iteration=iteration,
            model=model,
    ...
    # Print final summary
    print(f"\n{'='*70}")
    print(f"Self-Training Summary (eGeMAPS, Seed {seed})")
    print(f"{'='*70}")
